...
export default class AdminTherapistSessionsSummary extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  async componentDidMount() {
    ...
    if (this.state.user.role === "USER") {
      await this.getPatientSessions();
    } else if (this.state.user.role === "THERAPIST") {
      await this.getTherapistSessions();
    }
  ...
  getTherapistSessions = async () => {
    ...
    await fire
      .database()
      .ref(`SessionsSummary/${this.state.userId}`)
      .on("value", snap => {
        if (snap.exists()) {
          sessionsList = Object.values(snap.val());
          sessionsList = Object.values(sessionsList[0]);
          // alert(JSON.stringify(sessionsList.length));
  ...
  getPatientSessions = async () => {
    ...
    let sessionsList = [];
    let therapistId = this.state.user.therapists.filter(
      therapist => therapist.status === "active"
    )[0].id;
    // alert(this.state.userId)
    await fire
      .database()
      .ref(`SessionsSummary/${therapistId}/ ${this.state.userId}`)
      .on("value", snap => {
        // alert(snap.val())
        if (snap.exists()) {
          sessionsList = Object.values(snap.val());
        ...
        <View style={styles.fillSpace}>
          ...
          >
            {[
              ...
              {
                label: "Feedback:",
                value: this.state.selectedSession.feedback
              }
            ].map((item, index) => (
              <>
                {index !== 0 && (
